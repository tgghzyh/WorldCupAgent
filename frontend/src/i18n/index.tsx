...
export function I18nProvider({ children, defaultLocale = "en" }: I18nProviderProps) {
  ...
  const [translations, setTranslations] = useState<Translations>(
    defaultLocale === "zh" ? zhTranslations : enTranslations
  );

  // Sync translations when locale changes
  useEffect(() => {
    setTranslations(locale === "zh" ? zhTranslations : enTranslations);
    // Persist to localStorage
    if (typeof window !== "undefined") {
      localStorage.setItem("wc2026-locale", locale);
    }
  }, [locale]);
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      const saved = localStorage.getItem("wc2026-locale") as Locale | null;
