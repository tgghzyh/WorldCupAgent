...
- 不提交 `.env.local`、LLM API Key、SSH 私钥或云服务器密码。
- 保持 GitHub Deploy Key 为只读；服务器退役或怀疑泄露时，应删除旧密钥并创建新密钥。
- 面向正式用户发布前，应绑定域名并配置 HTTPS。这需要 TLS 反向代理或 Web 服务器、入站 TCP 443 规则和有效证书。若服务器位于中国内地，使用域名对外提供服务前应完成适用的 ICP 备案流程。
- 当前站点为静态展示。若需要在线生成预测，应新增受保护的后端服务，并且仅在服务端保存 API 凭据。
- 清理 Docker 旧镜像前，应先确认不再需要用于回滚；可定期审查后运行 `sudo docker image prune`。

## 9. 经验总结

1. 阿里云 Node.js 应用镜像提供的 Node.js 14.15.2 版本过旧，无法支持本项目使用的 Next.js 16。
2. Docker 能够解决运行时版本控制问题，但在 1 GiB 内存服务器上直接构建现代 Next.js 项目并不实际。
3. 项目页面均可预渲染，因此“本地静态导出 + Nginx 托管”是当前只读公开网站更简单、成本更低、稳定性更高的方案。
